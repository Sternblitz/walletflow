...
export function DynamicRouteManager({ clientId, clientSlug }: DynamicRouteManagerProps) {
    ...
    // Auto-create dynamic route if it doesn't exist
    useEffect(() => {
        if (!loading && !route && clientId && clientSlug) {
            createRoute()
        }
    }, [loading, route, clientId, clientSlug])
        ...
        try {
            const res = await fetch('/api/dynamic-routes', {
                ...
                body: JSON.stringify({
                    clientId,
                    clientSlug,
                    targetUrl: getStartURL(clientSlug)
                })
            })
            const data = await res.json()
            if (data.route) {
    ...
    const updateRoute = async () => {
        ...
        try {
            const res = await fetch('/api/dynamic-routes', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ clientId, targetUrl: newTargetUrl.trim() })
            })
            const data = await res.json()
            if (data.route) {
                setRoute(data.route)
            ...
            <div className="rounded-xl border border-white/10 bg-zinc-900/50 p-6">
                <div className="flex items-center justify-center py-8 gap-3">
                    ...
                    <span className="text-zinc-400 text-sm">
                        {creating ? 'Erstelle dynamischen QR-Code...' : 'Lade...'}
                    </span>
                </div>
            </div>
        )
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-zinc-400">
                            Dynamischer QR-Code - permanent & anpassbar
                        </p>
                    </div>
                </div>
                ...
                <div className="p-4 rounded-lg bg-black/50 border border-white/5 space-y-4">
                    {route ? (
                            ...
                            <div className="pt-4 border-t border-white/5 space-y-3">
                                <div className="flex items-center justify-between">
                                    ...
                                    <Button
                                        ...
                                        onClick={() => {
                                            setNewTargetUrl(route.target_url)
                        ...
                        <div className="text-center py-8">
                            ...
                            <p className="text-sm text-zinc-400">Erstelle dynamischen QR-Code...</p>
                        </div>
                    )}
                </div>
            </div>
                ...
                <DialogContent className="bg-zinc-950 border-white/10">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-zinc-400">
                            Gib die vollständige URL ein, zu der der QR-Code weiterleiten soll.
                        </DialogDescription>
                    </DialogHeader>

                    <div className="space-y-4 py-4">
                        <div className="space-y-2">
                            <label className="text-sm text-zinc-400">Ziel-URL</label>
                            <Input
                                value={newTargetUrl}
                                onChange={(e) => setNewTargetUrl(e.target.value)}
                                placeholder="https://start.getqard.com/neue-kampagne"
                                className="bg-zinc-900 border-white/10 text-white font-mono text-sm"
                            />
                        </div>
                        ...
                        <div className="text-xs text-zinc-500">
                            ...
                            <ul className="mt-1 space-y-1 ml-3">
                                <li>• https://start.getqard.com/neue-kampagne</li>
                                <li>• https://deine-website.de/aktion</li>
                                <li>• https://instagram.com/dein-profil</li>
                            </ul>
                        </div>
                    </div>
                    ...
                    <DialogFooter>
                        ...
                        <Button
                            onClick={updateRoute}
                            disabled={saving || !newTargetUrl.trim()}
